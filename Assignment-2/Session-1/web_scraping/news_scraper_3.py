...
def process_and_insert_news_data(connection, publisher_website, publisher, title, reporter, news_datetime, category, images, news_link):
    """
    ...
    try:
        ...
        # Insert news article
        news_id = insert_news(connection, category_id, reporter_id, publisher_id, news_datetime, title, news_body, news_link)
...
def single_news_scraper(news_link):
    session = HTMLSession()
    try:
        response = session.get(news_link)
        response.html.render()  # This will download Chromium if not found
        # time.sleep(3)

        # print(response.html.html)
        publisher_website = news_link.split('/')[2]       
...
if __name__ == "__main__":
    ...
    if conn is not None:
        news_link = "https://www.prothomalo.com/bangladesh/capital"
        ...
        session = HTMLSession()
        main_response = session.get(news_link)
